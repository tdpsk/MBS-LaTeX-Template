...
\global\def\mytitle{Title of the Paper}
\global\def\myauthor{Max Mustermann}
\global\def\mydate{December 2019}
\global\def\mylecturer{Lecturer Name} % This will become the academic
                                        % supervisor if the "isthesis"
                                        % flag is set to true
\global\def\papertype{Type of the Paper}
\global\def\course{Title of the Course}
\global\def\address{Address of Author}

% The following commands only apply to the thesis type
\global\def\degree{Master of Business Administration}
\global\def\supervisor{Max Supervisor}
...
\global\toggletrue{usestatutory}
\global\togglefalse{isthesis} % if this is set to true, the paper will
